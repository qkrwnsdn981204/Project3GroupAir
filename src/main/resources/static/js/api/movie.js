    ...
    movieData.movieListResult.movieList.forEach(movie => {
        ...
        movie.companys.forEach(company => {
            const row = `
                <tr>
                    <td>${movie.movieCd}</td>
                    <td class="movieNm-cell" style="color:yellow";>${movie.movieNm}</td>
...
$('#movie_table').on('click', '.movieNm-cell', function() {
    ...
    movieDetailFns(movieCd); // 함수 호출
    // 추가 작업 수행
});

//모달창으로 상세조회
function movieDetailFns(movieCd) {
    ...
    $.ajax({
        ...
        success: function(response) {
         ...
         console.log(response);
            ...
            const actors = movieDetail.movieInfoResult.movieInfo.actors.map(actor => actor.peopleNm).join(', ');
            const director = movieDetail.movieInfoResult.movieInfo.directors.map(director => director.peopleNm).join(', ');
            $('.detailModal1').css('display', 'block');
            $('.kr').text(movieNm);
            $('.en').text(movieNmEn);
            $('.open').text(openDt);
